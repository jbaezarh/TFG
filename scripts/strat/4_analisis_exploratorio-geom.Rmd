...
```{r}
g = ggarrange(g_dia,g_mes,g_year,ncol=1,common.legend = T,legend = "right")

annotate_figure(g,top=text_grob("Distribución temporal de fire",size=18))
```
...
g0 = ggplot(data = prov) + 
  ...
  ggtitle("Casos negativos") +
  theme_minimal()

g = ggarrange(g1,g0,nrow=2)
annotate_figure(g,top=text_grob("Distribución espacial de fire",size=19))
```
...
ggplot(data = prov) + 
  geom_sf() +
  geom_sf(data = datos, aes(color=dist_rios, alpha=dist_rios)) +
    scale_color_gradientn(colours = rainbow(5,rev=T)) +
  guides(size="none",alpha="none") +
  ggtitle("Distribución espacial de dist_rios") +
  labs(color = "Distancia a río") +
  theme_minimal() +
  theme(legend.position = "bottom",legend.key.width = unit(3,"line"),legend.title.position = "top")
```
...
uso_suelo = datos |> 
  ggplot(aes(x = uso_suelo,fill=fire)) +
  geom_bar(position = "dodge", alpha = 0.8) +
  scale_fill_hue(direction = - 1) +
  theme_minimal() +
